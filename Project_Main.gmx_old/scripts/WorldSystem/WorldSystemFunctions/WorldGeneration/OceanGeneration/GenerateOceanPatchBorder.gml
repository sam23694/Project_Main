///GenerateOceanPatchBorder(oceanIndex, oceanNum, coverage, smooth, borderOffset)
...
show_debug_message("Oceans: "+string(argument1)+" Coverage: "+string(argument2)+" OceanRegions: "+string(totalOcean)+" Regions per Ocean: "+string(regPerOcean));

var ox, oy, oceanRegions, strike, ex, ey, dx, dy, dis, valid;
...
for(var i = 0; i < argument1; i++)//for each ocean
{
    ...
    do
    {
        ...
        if(ox <= argument4 || ox >= World.mapWidth-argument4 && oy <= argument4 || oy >= World.mapHeight-argument4)
            valid = true;
    }until(valid || strike == allowedStrikes)
    if(valid)
    {
        ...
        {
            show_debug_message("Warning:GenerateOceanPatchBorder: Ocean placement not valid for ocean"+string(i)+"/"+string(argument1));
