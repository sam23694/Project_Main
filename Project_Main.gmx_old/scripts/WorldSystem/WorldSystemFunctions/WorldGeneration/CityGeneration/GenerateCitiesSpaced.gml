...
for(i = 0; i < argument0; i++)
{
    ...
    do
    {
        ...
        show_debug_message("City placement attempt at "+string(cx)+", "+string(cy));
            ...
            || WorldRegionHasCity_xy(cx, cy))
        {
            valid = false;
            strikes++;
            show_debug_message("Warning: Failed. Ocean or city occupying region.");
    ...
    if(valid == true)
    {
        ...
        {
            show_debug_message("Warning: Failed. Exceeded strikes with "+string(currentCities)+" cities");
            DebugSystem.warnings++;
        }
}
...
show_debug_message("City generation complete. Cities: "+string(currentCities)+" | Fails: "+string(oceanFails+disFails)+" | Ocean fails: "+string(oceanFails)+" | distance fails: "+string(disFails));
