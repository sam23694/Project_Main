...
{
    DebugWarning("GenerateBiomeStandard: biome cache was not defined, using all biomes.");
...
for(i = 0; i < World.mapWidth; i++)
{
    for(j = 0; j < World.mapHeight; j++)
    {
        ...
        DebugLog("Region "+string(i)+", "+string(j)+" biome set to "+string(WorldGetRegionBiome_xy(i, j).name));
    }
}

DebugLog("Generated as Standard");
